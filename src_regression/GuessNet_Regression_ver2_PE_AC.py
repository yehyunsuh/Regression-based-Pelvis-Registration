...
class CustomDataset(Dataset):
    def __init__(self, image_path_array, transform=None, img_size=128, pe_channels=2):
        ...
        # Generate actual image coordinates
        self.x_coords, self.y_coords = self.generate_xy_coordinates(img_size)

    def generate_xy_coordinates(self, img_size):
        """
        Generate X and Y coordinate channels where:
        - X-coordinates range from 0 to img_size - 1 across width.
        - Y-coordinates range from 0 to img_size - 1 across height.
        """
        x_coords = torch.arange(img_size).unsqueeze(0).repeat(img_size, 1)  # Shape: (H, W)
        y_coords = torch.arange(img_size).unsqueeze(1).repeat(1, img_size)  # Shape: (H, W)

        x_coords = x_coords.unsqueeze(0).float()  # Shape: (1, H, W)
        y_coords = y_coords.unsqueeze(0).float()  # Shape: (1, H, W)

        return x_coords, y_coords  # Each of shape (1, H, W)
    ...
    def __getitem__(self, idx):
        ...
        image = torch.cat([image, pos_embedding, x_coords, y_coords], dim=0)
        image_baseline = torch.cat([image_baseline, pos_embedding, x_coords, y_coords], dim=0)
...
def main(args, DEVICE):
    ...
    model = RegressionModel(model_name=args.model, num_outputs=6, pe_channels=(pe_channels+2)*2).to(DEVICE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vis_dir", type=str, default="GuessNet_Regression_baseline_ver2_PE_AC", help="Path to the visualization directory")
    parser.add_argument("--model_dir", type=str, default="../model_weights", help="Path to save the model directory")
    parser.add_argument("--model_name", type=str, default="GuessNet_Regression_baseline_ver2_PE_AC.pth", help="Path to save the model")
