...
def load_model(weight_path, model_name="resnet18", num_outputs=3, DEVICE="cpu", pe_channels=10):
    ...
    model = RegressionModel(model_name=model_name, num_outputs=6, pe_channels=(pe_channels+2)*2).to(DEVICE)
...
def generate_xy_coordinates(img_size):
    """
...
def test_model(args, images, images_baseline, DEVICE, experiment_type):
    ...
    x_coords, y_coords = generate_xy_coordinates(images.shape[1])

    images = torch.cat([images, pos_embedding, x_coords, y_coords], dim=0)
    images_baseline = torch.cat([images_baseline, pos_embedding, x_coords, y_coords], dim=0)
